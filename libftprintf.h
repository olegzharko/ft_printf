...
int					fun_pars_key_pr(char *key, int start,
					t_pars *stc, va_list ap);
...
char				*ft_itoa_base_ux_size_t(unsigned value, unsigned base);
int					is_flags(char key);
int					is_width(char key);
int					is_precition(char key);
int					is_len(char key);
...
wchar_t				*ft_strsub_wchar_t(wchar_t *str, unsigned start,
					size_t len);
