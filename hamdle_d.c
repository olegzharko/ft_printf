...
char				*type_of_int(t_pars *stc, va_list ap)
{
	...
	value = ch_val_of_int(stc, ap);
	stc->minus = value < 0 ? 1 : 0;
	value = value < 0 ? ((-1) * value) : value;
	result = ft_itoa_base_ux(value, 10);
	if (result[0] == '0' && stc->flag_plus == 0 && stc->z == 0 && stc->h == 0 &&
		stc->hh == 0 && stc->l == 0 && stc->ll == 0 && stc->j == 0 &&
		stc->z == 0 && stc->flag_minus == 0 && stc->precition == 0 &&
		stc->precition_on != 1)
		result[0] = '0';
